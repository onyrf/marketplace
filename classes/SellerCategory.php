...
class SellerCategory extends ObjectModel
{
    ...
    public static function getSelectedCategories($id_shop) {
        return Db::getInstance()->executeS('SELECT id_category FROM '._DB_PREFIX_.'seller_category WHERE id_shop = '.(int)$id_shop);
    }
    
    public static function deleteSelectedCategories($id_shop)
    {
        return Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'seller_category` WHERE id_shop = '.(int)$id_shop);
    }
    
    public static function isSellerCategory($id_category, $id_shop) {
        return Db::getInstance()->getValue('SELECT COUNT(*) FROM '._DB_PREFIX_.'seller_category WHERE id_category = '.(int)$id_category.' AND id_shop = '.(int)$id_shop);
    }
}