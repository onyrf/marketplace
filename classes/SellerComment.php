...
class SellerComment extends ObjectModel
{
	...
	public static function getBySeller($id_seller, $p = 1, $n = null, $id_customer = null)
	{
		if (!Validate::isUnsignedId($id_seller))
			return false;
		$validate = (int)Configuration::get('MARKETPLACE_MODERATE_COMMENTS');
		$p = (int)$p;
		$n = (int)$n;
		if ($p <= 1)
			$p = 1;
	...
	public static function getByOrder($id_order, $p = 1, $n = null, $id_customer = null)
	{
		...
		$cache_id = 'SellerComment::getByOrder'.(int)$id_order.'-'.(int)$p.'-'.(int)$n.'-'.(int)$id_customer.'-'.(bool)$validate;
		if (!Cache::isStored($cache_id))
		{
			$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT pc.`id_seller_comment`,
			IF(c.id_customer, CONCAT(c.`firstname`, \' \',  LEFT(c.`lastname`, 1)), pc.customer_name) customer_name, pc.`content`, pc.`grade`, pc.`date_add`, pc.title
			  FROM `'._DB_PREFIX_.'seller_comment` pc
			LEFT JOIN `'._DB_PREFIX_.'customer` c ON c.`id_customer` = pc.`id_customer`
			WHERE pc.`id_order` = '.(int)($id_order).($validate == '1' ? ' AND pc.`validate` = 1' : '').'
			ORDER BY pc.`date_add` DESC
			'.($n ? 'LIMIT '.(int)(($p - 1) * $n).', '.(int)($n) : ''));
			Cache::store($cache_id, $result);
		}
	...
	public static function getByCustomerOrder($id_seller, $id_customer, $get_last = false, $id_guest = false,$id_order)
	{
		$cache_id = 'SellerComment::getByCustomerOrder_'.(int)$id_seller.'-'.(int)$id_customer.'-'.(bool)$get_last.'-'.(int)$id_guest;
		if (!Cache::isStored($cache_id))
		{
			$results = Db::getInstance()->executeS('
				SELECT *
				FROM `'._DB_PREFIX_.'seller_comment` pc
				WHERE pc.`id_seller` = '.(int)$id_seller.' AND pc.`id_order` ='.(int)$id_order.'
				AND '.(!$id_guest ? 'pc.`id_customer` = '.(int)$id_customer : 'pc.`id_guest` = '.(int)$id_guest).'
				ORDER BY pc.`date_add` DESC '
				.($get_last ? 'LIMIT 1' : '')
			);
	...
	public static function getGradeBySeller($id_seller, $id_lang)
	{
		if (!Validate::isUnsignedId($id_seller) ||
			...
			return false;
		$validate = Configuration::get('MARKETPLACE_MODERATE_COMMENTS');
	...
	public static function getRatings($id_seller)
	{
		...
		/*$sql = 'SELECT (SUM(pc.`grade`) / COUNT(pc.`grade`)) AS avg,
			...
			AND pc.`deleted` = 0'.
			($validate == '1' ? ' AND pc.`validate` = 1' : '');*/
		$sql = 'SELECT (SELECT COUNT(pc.`grade`)
			FROM `'._DB_PREFIX_.'seller_comment` pc
			WHERE pc.`id_seller` = '.(int)$id_seller.' AND pc.`grade` >= 4 AND pc.`deleted` = 0' .
			($validate == '1' ? ' AND pc.`validate` = 1' : '') . ') / (COUNT(pc.`grade`)) * 5 AS avg,
				MIN(pc.`grade`) AS min,
				MAX(pc.`grade`) AS max
			FROM `'._DB_PREFIX_.'seller_comment` pc
			WHERE pc.`id_seller` = '.(int)$id_seller.'
	...
	public static function getPositifComment($id_seller)
	{
		$validate = Configuration::get('MARKETPLACE_MODERATE_COMMENTS');

		/*$sql = 'SELECT (SUM(pc.`grade`)*100 / (COUNT(pc.`grade`)*5)) AS positif		
			FROM `'._DB_PREFIX_.'seller_comment` pc
			WHERE pc.`id_seller` = '.(int)$id_seller.'
			AND pc.`deleted` = 0'.
			($validate == '1' ? ' AND pc.`validate` = 1' : '');*/
		$sql = 'SELECT (SELECT COUNT(pc.`grade`) * 100
			FROM `'._DB_PREFIX_.'seller_comment` pc
			WHERE pc.`id_seller` = '.(int)$id_seller.' AND pc.`grade` >= 4 AND pc.`deleted` = 0' .
			($validate == '1' ? ' AND pc.`validate` = 1' : '') . ') / (COUNT(pc.`grade`)) AS positif		
			FROM `'._DB_PREFIX_.'seller_comment` pc
			WHERE pc.`id_seller` = '.(int)$id_seller.'
			AND pc.`deleted` = 0'.
			($validate == '1' ? ' AND pc.`validate` = 1' : '');
	...
	{
		$sql  = '
			SELECT pc.`id_seller_comment`, pc.`id_seller`, IF(c.id_customer, CONCAT(c.`firstname`, \' \',  c.`lastname`), pc.customer_name) customer_name, pc.`title`, pc.`content`, pc.`grade`, pc.`date_add`, s.`name`, pc.`validate`
			FROM `'._DB_PREFIX_.'seller_comment` pc
			LEFT JOIN `'._DB_PREFIX_.'customer` c ON (c.`id_customer` = pc.`id_customer`)
    ...
    public static function isAlreadyComment($id_seller, $id_customer, $id_guest)
	{
		return (bool)Db::getInstance()->getValue('
			SELECT COUNT(*)
	...
	public static function isAlreadyCommentOrder($id_seller, $id_customer, $id_guest,$id_order)
	{
		return (bool)Db::getInstance()->getValue('
			SELECT COUNT(*)
			FROM `'._DB_PREFIX_.'seller_comment`
			WHERE `id_customer` = '.(int)$id_customer.'
                        AND `id_guest` = '.(int)$id_guest.'
			AND `id_seller` = '.(int)$id_seller .' AND `id_order` = '.(int)$id_order);
	}
