...
class SellerTransport extends ObjectModel
{
     ...
     */
    public static function getCarriers($id_lang, $active = false, $id_seller, $delete = false, $id_zone = false, $ids_group = null)
    {
    ...
    public static function getCarriersById($id_lang, $active = false, $id_carrier, $delete = false, $id_zone = false, $ids_group = null)
    {
            // Filter by groups and no groups => return empty array
            if ($ids_group && (!is_array($ids_group) || !count($ids_group)))
                    return array();
            ...
            $sql .= ' AND c.`id_carrier` = '.(int)$id_carrier;
            $sql .= ' GROUP BY c.`id_carrier` ORDER BY c.`position` ASC';
    ...
    public static function getCarriers1($id_lang, $active = false, $delete = false, $id_zone = false, $ids_group = null, $modules_filters = self::PS_CARRIERS_ONLY,$id_seller)
    {
        // Filter by groups and no groups => return empty array
        if ($ids_group && (!is_array($ids_group) || !count($ids_group))) {
            return array();
    ...
    public static function getCarriersForOrder($id_seller, $id_zone, $groups = null, $cart = null, &$error = array())
    {
        ...
        /*if (is_array($groups) && !empty($groups)) {
            $result = SellerTransport::getCarriers($id_lang, true, $id_seller, false, (int)$id_zone, $groups);
        } else {
            $result = SellerTransport::getCarriers($id_lang, true, $id_seller, false, (int)$id_zone, array(Configuration::get('PS_UNIDENTIFIED_GROUP')));
        }*/
        //$result = $this->getCarriers($id_lang, true, $id_seller);

        if (is_array($groups) && !empty($groups)) {
            $result = SellerTransport::getCarriers1($id_lang, true, false, (int)$id_zone, $groups, self::PS_CARRIERS_AND_CARRIER_MODULES_NEED_RANGE,$id_seller);
        } else {
            $result = SellerTransport::getCarriers1($id_lang, true, false, (int)$id_zone, array(Configuration::get('PS_UNIDENTIFIED_GROUP')), self::PS_CARRIERS_AND_CARRIER_MODULES_NEED_RANGE,$id_seller);
        }
    ...
    public static function getAllSellerCarriers($id_lang, $active = false, $delete = false, $id_zone = false, $ids_group = null)
    {
            ...
            $sql = '
            SELECT c.*,sc.id_seller, cl.delay
            FROM `'._DB_PREFIX_.'carrier` c
            LEFT JOIN `'._DB_PREFIX_.'carrier_lang` cl ON (c.`id_carrier` = cl.`id_carrier` AND cl.`id_lang` = '.(int)$id_lang.Shop::addSqlRestrictionOnLang('cl').')
            LEFT JOIN `'._DB_PREFIX_.'carrier_zone` cz ON (cz.`id_carrier` = c.`id_carrier`)'.
            ($id_zone ? 'LEFT JOIN `'._DB_PREFIX_.'zone` z ON (z.`id_zone` = '.(int)$id_zone.')' : '').'
    ...
    public static function getAllSellerCarriersWithNull($id_lang, $active = false, $delete = false, $id_zone = false, $ids_group = null)
    {
            // Filter by groups and no groups => return empty array
            if ($ids_group && (!is_array($ids_group) || !count($ids_group)))
                    return array();
    ...
    public static function isCarrierSeller($id_carrier) {
        return Db::getInstance()->getValue('SELECT COUNT(*) FROM '._DB_PREFIX_.'seller_carrier WHERE id_carrier = '.(int)$id_carrier);
    }

    public static function idSellerCarrier($id_seller) { 
        return Db::getInstance()->executeS('SELECT sc.id_carrier FROM '._DB_PREFIX_.'seller_carrier sc
            LEFT JOIN '._DB_PREFIX_.'carrier c ON (c.`id_carrier` = sc.`id_carrier`) 
            WHERE sc.id_seller = '.(int)$id_seller. ' AND c.`active` = 1');
    }

    public static function idSellerByCarrier($id_seller) { 
        $query ='SELECT sc.id_carrier FROM '._DB_PREFIX_.'seller_carrier sc
            LEFT JOIN '._DB_PREFIX_.'carrier c ON (c.`id_carrier` = sc.`id_carrier`) 
            WHERE sc.id_seller = '.(int)$id_seller. ' AND c.`active` = 1 AND c.deleted=0';

        /*if($selected_carrier)    
            $query .= ' AND sc.id_carrier = ' . $selected_carrier;*/
    ...
    public static function isSellerCarrier($id_seller, $id_carrier) {
        return Db::getInstance()->getValue('SELECT COUNT(*) FROM '._DB_PREFIX_.'seller_carrier WHERE id_seller = '.(int)$id_seller.' AND id_carrier = '.(int)$id_carrier);
    }
}