...
class AdminSellerEmailsController extends ModuleAdminController
{
    ...
    {
        $emails = SellerEmail::getAllEmails($this->context->language->id);
        ...
        $helper->listTotal = count($emails);
        $helper->actions = array('edit');
        $helper->show_toolbar = false;
        $helper->module = $this->module;
        $helper->identifier = 'id_seller_email';
        $helper->title = $this->l('Emails');
        $helper->table = 'seller_email';
        $helper->token = $this->token;
        $helper->currentIndex = 'index.php?controller=AdminSellerEmails';

        return $helper->generateList($emails, $fields_list).$this->displayFormResetEmails();
    }
    ...
    /*public function renderList()
    {
        ...
        if (Tools::isSubmit('submitFilter')) {          
            if (Tools::getValue('seller_emailFilter_reference') != '')
                $this->_where = 'AND a.reference LIKE "%'.pSQL(Tools::getValue('seller_emailFilter_reference')).'%"';
            
            if (Tools::getValue('seller_emailFilter_subject') != '')
                $this->_where = 'AND a.subject LIKE "%'.pSQL(Tools::getValue('seller_emailFilter_subject')).'%"';         
        }
        
        if (Tools::getValue('seller_emailOrderby')) {  
            $this->_orderBy = pSQL(Tools::getValue('seller_emailOrderby'));
            $this->_orderWay = pSQL(Tools::getValue('seller_emailOrderway'));
        }
