...
class marketplaceEditcarrierModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    public $id_seller;    
    ...
    public function postProcess() {
        ...
        if (Tools::isSubmit('submitEditCarrier')) {
            ...
            $id_carrier_old = pSQL(Tools::getValue('id_carrier'));
            ...
            if (count($this->errors) > 0) {
                ...
                $current_carrier = new Carrier((int)$id_carrier_old);
                ...
                {
                    if (file_exists(_PS_SHIP_IMG_DIR_.$current_carrier->id.'.jpg')) {
                            $image_name = _PS_SHIP_IMG_DIR_.$current_carrier->id.'.jpg';
                            $new_image = _PS_SHIP_IMG_DIR_.$carrier->id.'.jpg';
                            
                            copy($image_name, $new_image);
                            unlink($image_name);
                        }
                }
    ...
    public function initContent() {
        ...
        //$carrier = SellerTransport::getCarriersById($this->context->language->id, false, $id_carrier);
        ...
        $fees = $carrier->fees;
        $ranges_price = RangePrice::getRanges($id_carrier);
        $ranges_weight = RangeWeight::getRanges($id_carrier);
        ...
        foreach($ranges_weight as $rg)
        {
            $range_inf = $rg['delimiter1'];
            $range_sup = $rg['delimiter2'];
        }
        ...
        /*$tpl_vars = array();
        $fields_value = AdminCarrierWidzard::getStepThreeFieldsValues($carrier);
        AdminCarrierWidzard::getTplRangesVarsAndValues($carrier, $tpl_vars, $fields_value);*/
        ...
        $this->context->smarty->assign(array(
            ...
            'max_depth' => $carrier->max_depth,
            'max_weight' => $carrier->max_weight,
            'ranges' => $tpl_vars['ranges'],//$price_by_range,
            'price_by_range'=> $tpl_vars['price_by_range'],
            'fees' => $carrier->fees,
            'range_inf' => $range_inf,
            'range_sup' => $range_sup,
            ...
            'languages' => $languages,
            'id_lang' => $id_lang,
            'zones' => Zone::getZones(false),//$carrier->getZones(),
        ...
        $this->setTemplate('editcarrier.tpl');          
    }
